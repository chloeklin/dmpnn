...
| **3** | **HPG-hier** (true two-stage) — three-way vs wDMPNN / ChemArch on EA/IP | **seed-42 done ✓** (43/44 pending) |
...
- **Built & verified:** original HPG, within-group loss, seeded diagnostics, runners. HPG-hier (true two-stage) built, gates passed on Gadi. Stochastic-edge featurizer **shelved (not run)**.
- **SEED-42 RESULT IS IN (HPG-hier vs wDMPNN vs ChemArch, all splits, both targets, medians):**
  - **Chemistry (Monomer-heldout group-mean R²):** HPG-hier ≈ wDMPNN — IP at parity (0.97/0.97), EA within ~4% (0.93 vs 0.96); ChemArch drops to 0.81/0.85. The two-stage abstraction did **not** cost chemistry extrapolation.
  - **Architecture (ΔR²/ordering):** HPG-hier **> wDMPNN on every split**; ChemArch leads in-distribution (GD/PD) but HPG-hier **leads on Monomer-heldout**.
  - **Verdict:** HPG-hier is the best all-round model — dominates wDMPNN, more robust than ChemArch on unseen chemistry; not strictly best-of-both (ChemArch keeps the in-distribution architecture edge). **Single seed — directional, needs 43/44 for significance.**
  - **Consistency check passed:** fresh wDMPNN LOMO group-mean reproduces ~0.93. ChemArch fold-6 collapse is the known documented pathology (not a bug); its report "−0.37" summary is one-fold-dominated and should be re-derived.
- **Next action:** run **gate 2** (seed variance) → **seeds 43/44** for all three models (error bars + paired Wilcoxon) → optionally add Frac/GlobalArch/original-HPG for the full comparison → then layer `L_within` onto HPG-hier (Phase 4).
- **Deliverable:** `hpg_hier_progress_report.md` + `report_figures/` (supervisor update).
