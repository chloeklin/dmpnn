...
LOG_DIR="$LOCAL_PROJECT/logs/hpg_phase1_gates"
MANIFEST_DIR="$LOG_DIR/manifests"
TASK_LOG_DIR="$LOG_DIR/tasks"
mkdir -p "$MANIFEST_DIR" "$TASK_LOG_DIR"
MANIFEST="$MANIFEST_DIR/hpg_phase1_gates.manifest"
...
rm -f "$PBS_SCRIPT_DIR"/gate_*.pbs
PBS_SCRIPTS=()
task_index=0
while IFS=$'\t' read -r target_token model_token split fold task_args; do
    pbs_script="$PBS_SCRIPT_DIR/gate_$(printf '%02d' "$task_index")_${model_token}_${split}_f${fold}_${target_token}.pbs"
    cat > "$pbs_script" <<EOF
#!/bin/bash
#PBS -q $QUEUE
#PBS -P $PROJECT
...
#PBS -l jobfs=$JOBFS
#PBS -N gate_${model_token}_f${fold}
#PBS -r y
...
TASK_LOG_DIR="$PROJECT_DIR/logs/hpg_phase1_gates/tasks"
...
set -- $task_args
printf 'Gate task ${task_index}: model=${model_token} split=${split} fold=${fold} target=${target_token}\\n'
