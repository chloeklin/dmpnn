...
# Generate training scripts for insulator dataset with learning curve analysis
...
# insulator CAM-B3LYP targets
TARGETS=(
    "bandgap_chain"
)

# Train sizes for learning curve analysis
TRAIN_SIZES=("128" "256" "512" "768" "1024" "1536" "2048" "2560" "3072" "3367" )
...
generate_script() {
    ...
    local filename="train_insulator_${MODEL_NAME}_${target}${rdkit_suffix}${batch_norm_suffix}${size_suffix}_lc.sh"
...
#PBS -l jobfs=100GB
#PBS -N ${MODEL_NAME}_insulator_${target}${rdkit_suffix}${batch_norm_suffix}${size_suffix}_lc
...
# Main generation
echo "🚀 Generating ${MODEL_NAME} learning curve training scripts for insulator"
        ...
        if [[ "$SUBMIT_JOBS" == "true" ]]; then
            ...
            for target in "${TARGETS[@]}"; do
                for train_size in "${TRAIN_SIZES[@]}"; do
                    ...
                    if [[ "$RDKIT_ONLY" != "true" ]]; then
                        script_name="train_insulator_${MODEL_NAME}_${target}${size_suffix}_lc.sh"
                    ...
                    if [[ "$NO_RDKIT_ONLY" != "true" ]]; then
                        script_name="train_insulator_${MODEL_NAME}_${target}_rdkit${size_suffix}_lc.sh"
                        if [[ -f "$script_name" ]]; then
                            job_id=$(qsub "$script_name")
                            echo "✅ Submitted: $script_name -> $job_id"
                            ((++submitted_count))
            ...
            for target in "${TARGETS[@]:0:2}"; do  # Show first 2 targets as examples
                for train_size in "${TRAIN_SIZES[@]:0:3}"; do  # Show first 3 sizes as examples
                    ...
                    if [[ "$RDKIT_ONLY" != "true" ]]; then
                        echo "qsub train_insulator_${MODEL_NAME}_${target}${size_suffix}_lc.sh"
                    fi
                    if [[ "$NO_RDKIT_ONLY" != "true" ]]; then
                        echo "qsub train_insulator_${MODEL_NAME}_${target}_rdkit${size_suffix}_lc.sh"
                    fi
                done
            done
