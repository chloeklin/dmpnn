...
# Generate training scripts for opv_camb3lyp dataset with learning curve analysis using tabular models
...
generate_script() {
    ...
    local filename="train_opv_camb3lyp_tabular_${target}${size_suffix}_lc.sh"
...
#PBS -l jobfs=100GB
#PBS -N tabular_opv_${target}${size_suffix}_lc
...
if [[ "$SUBMIT_JOBS" == "true" ]]; then
    ...
    for script in "${OUTPUT_DIR}"/train_opv_camb3lyp_tabular_*_lc.sh; do
        if [[ -f "$script" ]]; then
