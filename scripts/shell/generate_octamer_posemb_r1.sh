#!/bin/bash
# Generate PBS scripts for the octamer position-embedding ablation on R1.
#
# This arm tests factor 2 of HANDOFF §7: the 8 learned position vectors in the
# octamer sequence encoder.  It is a follow-up to the K=1 arm, which excluded
# factor 5.  Both splits are pre-registered in
# analysis/model_diagnostics/PREREG_octamer_posemb_2026-08-05.md.
#
# This script creates the R1 (monomer_heldout) jobs only.  R3 is deliberately
# gated: do not generate or submit R3 until the R1 pilot has been verified and
# the R1 arm has been analysed.
#
...
PILOT_MANIFEST="$MANIFEST_DIR/oct_posemb_r1_pilot.manifest"
REMAINDER_MANIFEST="$MANIFEST_DIR/oct_posemb_r1_after_review.manifest"
: > "$FULL_MANIFEST"
...
# Pre-flight comparator checks run against the local copy of the results.
    ...
    for target_index in "${!TARGET_TOKENS[@]}"; do
        ...
        for seed in "${SEEDS[@]}"; do
            for fold in {0..8}; do
                ...
                # Pre-flight: the __noposemb output must not already exist, and
                # the K=16 comparator must be present.
...
OUTPUT="\${PAYLOAD%%|*}"
ARGS="\${PAYLOAD#*|}"
# The runner writes the base filename (without the __noposemb token). We move
...
while IFS= read -r _line; do
    task_pbs="$PBS_DIR/oct_posemb_r1_p_${TASK_INDEX}.pbs"
    write_per_task_pbs "$PILOT_MANIFEST" "$TASK_INDEX" "oct_posemb_r1_p_${TASK_INDEX}" "$task_pbs"
...
TASK_INDEX=0
while IFS= read -r _line; do
    task_pbs="$PBS_DIR/oct_posemb_r1_r_${TASK_INDEX}.pbs"
    write_per_task_pbs "$REMAINDER_MANIFEST" "$TASK_INDEX" "oct_posemb_r1_r_${TASK_INDEX}" "$task_pbs"
...
printf 'Octamer position-embedding ablation (R1) cells: %s\n' $((1 * 2 * 9))
printf 'Octamer position-embedding ablation (R1) runs: %s\n' "$TOTAL_RUNS"
...
printf '\n=== Submission runbook (R1) ===\n'
printf '1. Generator was just run from: %s\n' "$LOCAL_PROJECT"
printf '2. Check remaining SU: nci_account -P %s\n' "$PROJECT"
printf '3. Submit the two R1 pilots one at a time and inspect sidecars:\n'
printf '     qsub %s\n' "${PILOT_PBS_LIST[0]}"
...
printf '     qsub %s\n' "${PILOT_PBS_LIST[1]}"
printf '4. Submit the R1 remainder only after both pilots pass:\n'
printf '     for f in %s/oct_posemb_r1_r_*.pbs; do qsub "$f"; done\n' "$PBS_DIR"
printf '5. Gate before R3: do not run scripts/shell/generate_octamer_posemb_r3.sh\n'
printf '   until the R1 pilot sidecars are verified and the R1 arm has been analysed.\n'
printf '   Rationale: if R1 shows a large effect, R3 provides generalisation evidence;\n'
printf '   if R1 shows nothing, R3 is cheap confirmation of a null rather than a\n'
printf '   second independent gamble.\n'
printf '6. Cost estimate is unverified for this arm: recent wDMPNN re-runs came in well\n'
printf '   below projection.\n'
