...
def grouped_metrics(frame: pd.DataFrame) -> dict:
    ...
    return {"group_mean_r2": r2(group.y_true, group.y_pred), "overall_r2": r2(frame.y_true, frame.y_pred), "mae": float(np.abs(frame.y_true - frame.y_pred).mean()), "rmse": float(np.sqrt(np.square(frame.y_pred - frame.y_true).mean())), "bias": float((frame.y_pred - frame.y_true).mean())}
