...
def parse_experiment_name(exp_name: str) -> Dict[str, str]:
    ...
    Format: {dataset}__{target}__{desc_suffix}__{rdkit_suffix}__rep{i}
    Example: insulator__bandgap_chain__rdkit__rep0
    """
...
def find_available_experiments(checkpoint_dir: Path, preprocessing_dir: Path) -> List[Dict[str, str]]:
    ...
    for model_dir in checkpoint_dir.iterdir():
        ...
        for exp_dir in model_dir.iterdir():
            ...
            preprocess_path = preprocessing_dir / model_name / exp_name
...
def group_experiments_by_dataset_model(experiments: List[Dict[str, str]]) -> Dict[Tuple[str, str, bool, bool], List[Dict[str, str]]]:
    """Group experiments by (dataset, model, has_desc, has_rdkit) for batch evaluation."""
...
def run_evaluation(dataset: str, model: str, has_desc: bool, has_rdkit: bool, 
    ...
    try:
        ...
        if verbose:
            print(f"✅ Success: {dataset} {model} (desc={has_desc}, rdkit={has_rdkit})")
        return True
    except subprocess.CalledProcessError as e:
        print(f"❌ Failed: {dataset} {model} (desc={has_desc}, rdkit={has_rdkit})")
        if verbose:
            print(f"Error: {e}")
            if e.stdout:
...
def main():
    ...
    parser.add_argument('--model', type=str, choices=['DMPNN', 'wDMPNN'],
                       help='Only run evaluations for specific model')
    ...
    if args.verbose:
        print("\nExperiment summary:")
        for (dataset, model, has_desc, has_rdkit), exps in groups.items():
            print(f"  {dataset} + {model}: {len(exps)} replicates (desc={has_desc}, rdkit={has_rdkit})")
