...
def get_fold_assignments(df: pd.DataFrame, col_map: Dict) -> np.ndarray:
    """Get or create monomer-disjoint 9-fold LOMAO assignments.

    Priority:
      1. Inline 'fold' column in df
      2. Pre-computed splits JSON (ea_ip_aheldout_seed42.json, 9 folds)
         Maps original-dataset indices to positional indices in df via _orig_idx column.
      3. Fallback: GroupKFold(n_splits=9) on smiles_A
    """
    ...
    # Try loading from splits JSON — indices are relative to the original df
    ...
    if splits_json.exists():
        ...
        # Build mapping: original-dataset row index -> positional index in df
        if "_orig_idx" in df.columns:
            orig_to_pos = {int(orig): pos for pos, orig in enumerate(df["_orig_idx"].values)}
        else:
            orig_to_pos = {int(orig): pos for pos, orig in enumerate(df.index)}

        fold_arr = np.full(len(df), -1, dtype=int)
        for fold_info in splits_data["folds"]:
            for orig_idx in fold_info["test_indices"]:
                pos = orig_to_pos.get(int(orig_idx), -1)
                if pos >= 0:
                    fold_arr[pos] = fold_info["fold"]

        assigned = int((fold_arr >= 0).sum())
        if assigned == len(df):
            print(f"  Loaded LOMAO fold assignments from splits JSON "
                  f"({splits_data['n_folds']} folds, {assigned}/{len(df)} rows mapped)")
            return fold_arr
        else:
            print(f"  WARNING: splits JSON only covered {assigned}/{len(df)} rows "
                  f"-- falling back to GroupKFold(9).")

    # Fallback: LOMAO via GroupKFold(9) on smiles_A
    print("  Creating LOMAO folds (GroupKFold n=9 on smiles_A)...")
    sA_col = col_map["smiles_A"]
...
def generate_visualizations(
    ...
    for target in sorted(summary_df["target"].unique()):
        ...
        if len(core4_sub) > 0:
            ...
            ax.set_xlabel("Architecture-deviation prediction, R²(Δy)", fontsize=11)
            if target == "EA":
                ax.set_title(f"Electronic Affinity ({target})", fontweight="bold")
            else:
                ax.set_title(f"Ionisation Potential ({target})", fontweight="bold")
            ax.axvline(0, color="k", linewidth=0.5)
            
            # Add value labels with better positioning
            x_min, x_max = ax.get_xlim()
            x_range = x_max - x_min
            # Pre-expand xlim so labels for the longest bar don't clip the border
            max_bar = max(bar.get_width() for bar in bars)
            ax.set_xlim(x_min, max(x_max, max_bar + x_range * 0.18))
            x_min, x_max = ax.get_xlim()
            x_range = x_max - x_min
...
def main():
    ...
    # Preserve original row indices for LOMAO fold mapping (before any reset_index)
    df_matched = df_matched.copy()
    df_matched["_orig_idx"] = df_matched.index.astype(int)
