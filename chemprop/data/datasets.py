...
from chemprop.data.datapoints import MolAtomBondDatapoint, MoleculeDatapoint, ReactionDatapoint, PolymerDatapoint
...
class PolymerDatum(NamedTuple):
    """a singular training data point for polymers"""
...
PolymerDataset: TypeAlias = Dataset[PolymerDatum]
...
class PolymerDataset(_MolGraphDatasetMixin, MolGraphDataset):
    ...
    def __getitem__(self, idx: int) -> Datum:
        ...
        return PolymerDatum(mg, d.edges, self.V_ds[idx], self.X_d[idx], self.Y[idx], d.weight, d.lt_mask, d.gt_mask)
    ...
    def _init_cache(self):
        """initialize the cache"""
        # Use the data points directly since they contain both mol and edges
        self.mg_cache = (MolGraphCache if self.cache else MolGraphCacheOnTheFly)(
            # (self.mols, self.edges), self.V_fs, self.E_fs, self.featurizer
            self.data, self.V_fs, self.E_fs, self.featurizer
        )
