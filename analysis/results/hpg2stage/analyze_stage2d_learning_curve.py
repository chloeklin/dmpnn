...
OUT = ROOT / 'analysis' / 'results' / 'hpg2stage' / 'learning_curve_output'
...
def check_needs_inverse_transform(y_true, y_pred):
    """Check if predictions need inverse transform by comparing ranges."""
    yt_range = y_true.max() - y_true.min()
    yp_range = y_pred.max() - y_pred.min()
    # If pred range is much smaller than true range, likely normalized
    if yt_range > 0 and yp_range / yt_range < 0.3:
        return True
    # If R² is very negative, likely needs transform
...
def apply_inverse_transform(y_true, y_pred):
    """Apply linear-regression inverse transform."""
...
def load_all_predictions():
    ...
    # Also check for inverse transform parameters from the reference frac model
    # (HPG2Stage predictions, not HPG2Stage_LC)
    ref_pred_dir = ROOT / 'predictions' / 'HPG2Stage'
    inv_transform_params = {}  # (target, fold) -> (intercept, slope)
    
    for tgt_short, tgt_long in TARGETS.items():
        for fold in range(N_FOLDS):
            ref_file = ref_pred_dir / f'{DATASET_NAME}__{tgt_long}__copoly_stage2d_frac__a_held_out__split{fold}.npz'
            if ref_file.exists():
                npz = np.load(ref_file, allow_pickle=True)
                y_true_ref = npz['y_true'].flatten()
                y_pred_ref = npz['y_pred'].flatten()
                slope, intercept, _, _, _ = stats.linregress(y_pred_ref, y_true_ref)
                inv_transform_params[(tgt_long, fold)] = (intercept, slope)
            ...
            for frac_pct in FRACTIONS:
                for model in MODELS:
                    ...
                    if check_needs_inverse_transform(y_true, y_pred):
                        if (tgt_long, fold) in inv_transform_params:
                            intercept, slope = inv_transform_params[(tgt_long, fold)]
                            y_pred = y_pred * slope + intercept
                        else:
